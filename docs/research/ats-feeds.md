...
|---|---|---|---:|
| **BambooHR** | `https://{ref}.bamboohr.com/careers/list` | 200, `{meta, result[]}` — solace 26, caseiq 4, masv 2, welbi 1 | **31** |
| **Workable** | `https://apply.workable.com/api/v1/widget/accounts/{ref}?details=true` | 200 JSON (distillersr-inc) | **5** |
| **Recruitee** | `https://{ref}.recruitee.com/api/offers/` | 200, 167 offers (huaweicanada) | **5** |
| **BreezyHR** | `https://{ref}.breezy.hr/json` | 200 JSON (apption) | 2 |
...
| **Rippling** | `https://api.rippling.com/platform/api/ats/v1/board/{ref}/jobs` | 200, 28 jobs (pythian) | 1 |
...
|---|---|---:|
| **Workday** | `POST /wday/cxs/{tenant}/{site}/jobs` returned **422, not 404** — the endpoint is live and keyless, but needs a POST body, offset pagination, **and** a multi-segment ref (tenant / wd-number / site). The list stores tenant only (`mitel`, `ciena`), so every row needs its site captured before any code runs. This is the case ADR-0012's `board_ref` design was written for. | **28** |
